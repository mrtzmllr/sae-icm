...
"""Gemma-2-2b model with an SAE inserted similar to insert_sae_llama.py for Llama-3.2-1b"""
...
class SAEonGemma2(nn.Module):
    ...
    def forward(self, input_ids, attention_mask = None, labels = None):
        with torch.no_grad():
            ...
            # final tuple entry is post-RMSNorm, not a raw residual
            assert self.sae_layer + 1 < len(outputs.hidden_states) - 1
            x = outputs.hidden_states[self.sae_layer + 1]
...
def insert_sae(model, conf):
    ...
    if conf["model"]["retrieve_from_hub"]:
        model.model.sae = sae
        raise NotImplementedError
    
    elif conf["model"]["finetune_language_model"]:
        ...
        elif conf["sae"]["use_finetuned"]:
            if conf["model"]["run_eval"]: sae.from_finetuned_2(conf=conf)
            else: sae.from_finetuned(conf=conf)
