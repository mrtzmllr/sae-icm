...
class SAEMetricsCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, model=None, **kwargs):
        ...
        # This captures the pre-layer residual stream for Llama-3.2-1B experiments
        captured = {}
        hook_handle = sae.register_forward_pre_hook(
            lambda module, inputs: captured.update(x=inputs[0])
        )
        ...
        for i, batch in enumerate(self.validation_set):
            ...
            assert "x" in captured, "SAE forward pre-hook never fired - is the SAE spliced in?"
            x = captured["x"]
