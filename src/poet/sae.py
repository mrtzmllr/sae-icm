...
from src.poet.directories import finetuned_sae_states_dir, off_the_shelf_sae_states_dir, write_sae_dir
...
class TopKSAE(BaseSAE):
    def __init__(self, conf):
        ...
        self.write_gsm8k_heap = conf["interpretability"]["write_gsm8k_heap"]
    ...
    def intervene(self, post):
        ...
        if len(drops) != 1: assert len(drops) == len(includes)

        if includes != []:
            indices_to_include = post.indices.clone()
            values_to_include = post.values.clone()
            insert_value = self.insertion_value
            self.draw = 0
            for incl_count, incl in enumerate(includes):
                if self.step >= 0:
                    mask = (post.indices == drops[incl_count])
    ...
    def encoder(self, x_flat, return_pre = False):
        ...
        if self.run_eval:
            if self.do_intervene:
                if self.step > 0:
                    self.indices.append(new_indices.detach().cpu())
                    self.values.append(new_values.detach().cpu())
            elif self.write_gsm8k_heap:
                self.indices.append(post.indices.tolist())
                self.values.append(post.values.tolist())
            else:
                if self.step > 0:
                    self.indices.append(post.indices.detach().cpu())
                    self.values.append(post.values.tolist())
    ...
    def from_huggingface(self, conf):
        if "llama" in conf["model"]["name"].lower():
            if conf["sae"].get("pretrained_format", "saelens") == "llama-scope":
                return self._from_llama_scope(conf)
            return self._from_saelens(conf)
    ...
    def _from_saelens(self, conf):
        config_path, params_path = retrieve_sae_configs(conf=conf)

        with open(config_path) as f:
            sae_conf = json.load(f)

        assert sae_conf["d_in"] == self.d_model, (sae_conf["d_in"], self.d_model)
        assert sae_conf["d_sae"] == self.d_sae, (sae_conf["d_sae"], self.d_sae)
        assert sae_conf.get("normalize_activations", "none") == "none", \
            f"normalize_activations={sae_conf.get('normalize_activations')} not handled; weights would need folding."
        assert sae_conf.get("apply_b_dec_to_input", True), \
            "encoder assumes b_dec is subtracted from the input (apply_b_dec_to_input=True)."
        if sae_conf.get("k") != self.top_k:
            print(f"[SAE] pretrained k={sae_conf.get('k')} differs from configured top_k={self.top_k}; "
                  f"using configured top_k.")

        state = load_file(params_path)
        expected = {"W_enc", "b_enc", "W_dec", "b_dec"}
        missing = expected - set(state.keys())
        assert not missing, f"SAELens SAE missing keys {missing}; available keys: {list(state.keys())}"

        assert self.any_trainable, "Llama/SAELens loader assumes any_trainable=True (nn.Linear SAE)."

        with torch.no_grad():
            self.W_enc.weight.copy_(state["W_enc"].T.to(self.W_enc.weight.dtype).to(self.W_enc.weight.device))
            self.W_enc.bias.copy_(state["b_enc"].to(self.W_enc.bias.dtype).to(self.W_enc.bias.device))
            self.W_dec.weight.copy_(state["W_dec"].T.to(self.W_dec.weight.dtype).to(self.W_dec.weight.device))
            self.W_dec.bias.copy_(state["b_dec"].to(self.W_dec.bias.dtype).to(self.W_dec.bias.device))

        return self


    def _from_llama_scope(self, conf):
        config_path, params_path = retrieve_sae_configs(conf=conf)

        with open(config_path) as f:
            hp = json.load(f)

        assert hp["d_model"] == self.d_model, (hp["d_model"], self.d_model)
        assert hp["d_sae"] == self.d_sae, (hp["d_sae"], self.d_sae)
        assert self.any_trainable, "Llama Scope loader assumes any_trainable=True (nn.Linear SAE)."
        assert not hp.get("apply_decoder_bias_to_pre_encoder", False), \
            "loader assumes apply_decoder_bias_to_pre_encoder=False (the Llama Scope residual convention)."

        if hp.get("act_fn") not in ("top-k", "topk"):
            print(f"[SAE] Llama Scope SAE uses act_fn={hp.get('act_fn')} (trained top_k={hp.get('top_k')}); "
                  f"repurposing its weights as a TopK SAE with top_k={self.top_k}.")

        state = load_file(params_path)
        expected = {"encoder.weight", "encoder.bias", "decoder.weight", "decoder.bias"}
        missing = expected - set(state.keys())
        assert not missing, f"Llama Scope SAE missing keys {missing}; available keys: {list(state.keys())}"

        if hp.get("norm_activation") == "dataset-wise":
            avg = hp["dataset_average_activation_norm"]
            s_in = float(np.sqrt(self.d_model)) / avg["in"]
            s_out = float(np.sqrt(self.d_model)) / avg["out"]
        else:
            s_in = s_out = 1.0

        We = state["encoder.weight"].float()  # [d_sae, d_model]
        be = state["encoder.bias"].float()    # [d_sae]
        Wd = state["decoder.weight"].float()  # [d_model, d_sae]
        bd = state["decoder.bias"].float()    # [d_model]

        W_enc_lin = s_in * We
        b_enc_lin = be + (s_in / s_out) * (bd @ We.t())
        W_dec_lin = Wd / s_out
        b_dec_lin = bd / s_out

        with torch.no_grad():
            self.W_enc.weight.copy_(W_enc_lin.to(self.W_enc.weight.dtype).to(self.W_enc.weight.device))
            self.W_enc.bias.copy_(b_enc_lin.to(self.W_enc.bias.dtype).to(self.W_enc.bias.device))
            self.W_dec.weight.copy_(W_dec_lin.to(self.W_dec.weight.dtype).to(self.W_dec.weight.device))
            self.W_dec.bias.copy_(b_dec_lin.to(self.W_dec.bias.dtype).to(self.W_dec.bias.device))
    ...
    def from_finetuned(self, conf):
        state_dir = write_sae_dir(conf) + f"/checkpoint-{conf['eval']['checkpoint']}/"
        bin_file = state_dir + "pytorch_model.bin"
        st_file = state_dir + "model.safetensors"

        if os.path.exists(bin_file):
            ft_params = torch.load(bin_file, map_location="cpu", mmap=True)
            src = bin_file
        elif os.path.exists(st_file):
            ft_params = load_file(st_file)
            src = st_file
        else:
            raise FileNotFoundError(
                f"No stage-1 SAE checkpoint in {state_dir} (looked for pytorch_model.bin / model.safetensors). "
                f"Check eval.checkpoint={conf['eval']['checkpoint']} and that model/top_k/lr/orthogonality_lambda "
                f"match the stage-1 run (they determine write_sae_dir)."
            )

        required = ["sae.W_enc.weight", "sae.W_enc.bias", "sae.W_dec.weight", "sae.W_dec.bias"]
        missing = [k for k in required if k not in ft_params]
        assert not missing, f"{src} missing keys {missing}; available sae.* keys: {[k for k in ft_params if 'sae.' in k]}"
        ...
        print(f"[SAE] loaded stage-1 fine-tuned SAE from {src}")
        return self
    

    def from_finetuned_2(self, conf):
        # loads the SAE states written during LM fine-tuning
        sae_dir = finetuned_sae_states_dir(conf)
        state_file = sae_dir + f"/sae_state_{conf['eval']['checkpoint']}.pt"
...
def retrieve_sae_configs(conf):
    ...
    if conf["sae"]["type"] == "jump-relu":
        ...
        if "llama" in conf["model"]["name"].lower():
            repo_id = conf["sae"]["pretrained_repo"]
            subfolder = conf["sae"]["pretrained_subfolder"]
            fmt = conf["sae"].get("pretrained_format", "saelens")
            if fmt == "llama-scope":
                config_path = hf_hub_download(repo_id=repo_id, filename=f"{subfolder}/hyperparams.json")
                params_path = hf_hub_download(repo_id=repo_id, filename=f"{subfolder}/checkpoints/final.safetensors")
            else:
                config_path = hf_hub_download(repo_id=repo_id, filename=f"{subfolder}/cfg.json")
                params_path = hf_hub_download(repo_id=repo_id, filename=f"{subfolder}/sae_weights.safetensors")
